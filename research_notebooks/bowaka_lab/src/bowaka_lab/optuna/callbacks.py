"""Optuna callbacks for logging, progress, and degeneracy detection.

Mirrors ``research_notebooks/market_lab/pmm_dynamic/pmm_lab/optuna/callbacks.py``
verbatim — these classes are framework-agnostic.
"""
...
class DegeneracyCheckCallback:
    ...
    If after ``check_after_n_trials`` complete trials, fewer than
    ``min_distinct_values`` distinct objective values exist, log a warning.
    """
...
class TrialLoggingCallback:
    ...
    def __call__(self, study: optuna.Study, trial: optuna.trial.FrozenTrial) -> None:
        ...
        if n_complete % self.log_every == 0 or trial.number < 3:
            ...
            logger.info(
                f"Trial {trial.number}: value={trial_value:.4f} | "
