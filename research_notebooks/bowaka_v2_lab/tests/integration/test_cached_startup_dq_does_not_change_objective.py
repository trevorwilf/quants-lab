"""Walk-forward objective is unchanged by enabling the startup-DQ cache.

Speedup report v2 §4 P4 / §5.6 / Phase 3 task 6. Strongest objective
parity test: a 2-trial walk-forward study with the DQ cache off vs on
must produce the same ``best_value`` and ``best_params``.
"""
...
def _write_cfg(out_path: Path, *, lake: Path, symbols, cache: bool) -> Path:
...
@pytest.mark.slow
def test_walkforward_objective_parity_dq_cache_off_vs_on(tmp_path: Path) -> None:
    lake = tmp_path / "lake"
    symbols = ["AAA"]
    build_tiny_lake(lake, symbols, start=dt.date(2024, 1, 1), end=dt.date(2024, 5, 1))

    cfg_off = _write_cfg(tmp_path / "wf_off.yml", lake=lake, symbols=symbols, cache=False)
    cfg_on = _write_cfg(tmp_path / "wf_on.yml", lake=lake, symbols=symbols, cache=True)
