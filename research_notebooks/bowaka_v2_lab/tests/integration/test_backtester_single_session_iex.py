"""3-symbol × 1-session synthetic real-shape fixture; complete artifacts; non-empty trades."""
...
def test_single_session_three_symbols_iex(tmp_path: Path) -> None:
    paths = BowakaV2Paths(
    ...
    sessions = [sd]
    symbols = ["AAA", "BBB", "CCC"]
    bars_by_sym = {s: make_minute_bars(s, sd, minutes=30, drift_per_minute=0.7,
                                          minute_volume=20_000) for s in symbols}
    # Daily fixture with a strong move to trigger the target on exit-day.
    daily_by_sym = {s: make_daily_bars(s, _dt.date(2024, 9, 1), n_sessions=5,
    ...
    # Verify artifacts exist and trades happened (the strong daily move triggers the target).
    assert (result.run_dir / "trades.parquet").is_file()
    trades = pd.read_parquet(result.run_dir / "trades.parquet")
    # At least one trade should have closed on session 1 (target hit since daily range is 10%).
    assert len(trades) >= 1
