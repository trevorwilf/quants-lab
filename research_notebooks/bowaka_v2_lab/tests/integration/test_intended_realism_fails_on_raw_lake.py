"""The shipped intended-realism config fails closed against a raw lake.

Realism remediation 2 Phase 1 (audit §P0-005). The committed
``bowaka_v2_actual_sip_intended_realism.yml`` carries
``market_data.require_adjusted_daily_bars: true``; running it against a lake
whose manifest declares ``adjustment: raw`` must fail closed with
``adjustment_mismatch`` recorded as a required data-quality failure.
"""
...
def _build_raw_lake(root: Path, symbol: str, session: dt.date, *, feed: str) -> None:
    """A lake whose ``_ingestion/manifest.json`` declares ``adjustment: raw``."""
    ddates = [session - dt.timedelta(days=i) for i in range(80, -1, -1)]
    _write(
        layout.daily_bars_path(root, symbol, feed=feed),
        pd.DataFrame(
            {
    ...
    _write(
        layout.minute_bars_path(root, symbol, session.year, session.month, feed=feed),
        pd.DataFrame(
            {
    ...
    mpath.write_text(
        json.dumps({"feed": feed, "adjustment": "raw", "dataset_hashes": {"lake": "sha256:raw"}}),
        encoding="utf-8",
    )
    ...
    pd.DataFrame(
        [{
            "symbol": symbol, "feed": feed, "timeframe": "1d",
            ...
            "audit_run_id": f"audit_2024-09-01T000000Z_{feed}",
        }]
    ).to_parquet(audit_dir / f"audit_2024-09-01T000000Z_{feed}.parquet", index=False)
    write_lake_asset_master(root, [symbol])


def _load_shipped_for_fixture(lake_root: Path, session: dt.date) -> dict:
    """Load the shipped SIP intended-realism config, retargeted at the fixture lake."""
    cfg = load_config(_LAB_ROOT / "configs" / "bowaka_v2_actual_sip_intended_realism.yml")
    cfg.pop("_source_path", None)
    cfg["market_data"]["shared_root"] = str(lake_root)
    # Widen the universe price band — the fixture prices bars at $10 which is in
    # band, but ADV gating is not the point of this DQ test.
...
def test_intended_realism_config_fails_closed_on_raw_lake(tmp_path: Path) -> None:
    symbol, session = "AAA", dt.date(2024, 9, 4)
    lake = tmp_path / "lake"
    _build_raw_lake(lake, symbol, session, feed="sip")
    ...
    minute_supplier, daily_supplier = make_lake_suppliers(lake, feed="sip")
    daily_cache = {session: build_daily_cache_from_lake(lake, [symbol], session, feed="sip")}
