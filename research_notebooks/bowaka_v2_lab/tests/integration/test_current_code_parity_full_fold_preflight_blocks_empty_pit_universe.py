"""Phase 1 (audit 2026-05-29 §5.4) — parity preflight blocks an empty PIT universe.

A fold whose point-in-time universe resolves to fewer than
``preflight.min_pit_universe_per_fold`` (default 25) eligible symbols cannot
produce a meaningful objective; the full-fold preflight hard-fails it.
"""
...
def test_below_min_pit_universe_blocks_study(tmp_path: Path) -> None:
    _clear_full_fold_preflight_cache()
    lake = tmp_path / "lake"
    # Only ONE symbol -> PIT universe count = 1, below the default minimum 25.
    build_lake(
        lake, ["AAAA"],
    ...
    # default min_pit_universe_per_fold (25) — single symbol is well below it
    folds = [
    ...
    assert "empty_pit_universe" in str(ei.value)
    _clear_full_fold_preflight_cache()
