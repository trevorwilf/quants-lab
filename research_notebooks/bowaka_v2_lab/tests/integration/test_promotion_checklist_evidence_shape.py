"""Phase 8 Task 4 — every promotion-gate check records evidence as a dict.

The promotion gate result records structured ``evidence: dict`` per check, not
just a pass/fail boolean or a free-text string. This is verified both directly
(``run_all_checklists``) and end-to-end through the ``promotion-gate`` CLI.
"""
...
from bowaka_v2_lab.promotion.checklist import run_all_checklists
...
def test_every_check_has_dict_evidence(tmp_path: Path) -> None:
    result = _run(tmp_path)
    results = run_all_checklists(result.run_dir)
    assert results, "checklist produced no results"
    for item_id, (status, evidence) in results.items():
        assert status in ("pass", "fail", "unknown"), f"{item_id}: bad status"
        assert isinstance(evidence, dict), (
            f"{item_id}: evidence must be a dict, got {type(evidence).__name__}"
        )
        # Evidence is never empty — it always records *something* structured.
        assert evidence, f"{item_id}: evidence dict is empty"


def test_promotion_gate_cli_emits_dict_evidence(tmp_path: Path) -> None:
    result = _run(tmp_path)
    artifacts_root = tmp_path / "r" / "bowaka_v2_lab" / "artifacts"
    proc = subprocess.run(
    ...
    cr = payload["checklist_results"]
    assert cr, "promotion-gate emitted no checklist_results"
    for item_id, res in cr.items():
        assert set(res) >= {"status", "evidence"}, item_id
        assert isinstance(res["evidence"], dict), (
            f"{item_id}: CLI evidence must be a dict"
        )
