...
#: Substring of :data:`_CONFIG_HEADER` that marks a file as machine-generated by
#: the contract -> config mapper. Auto-discovery keys off this (not a hardcoded
#: filename list) so a newly-added generated config is picked up automatically
#: and a hand-tuned operator overlay (no marker) is never touched.
GENERATED_MARKER: str = "GENERATED bowaka_v2 lab config"


def recover_generation_args(path: Path, doc: dict[str, Any]) -> dict[str, str]:
    """Recover the ``import-actual-config`` args from a generated config's content.

    ``feed`` / ``mode`` / ``purpose`` are read back from the YAML
    (``market_data.feed`` / ``simulation.mode`` / ``run.kind``); ``purpose`` is
    ``optuna`` iff ``run.kind == "optuna"``. ``feed_thresholds`` is
    ``sip_tightened`` iff the ``--feed-thresholds sip_tightened`` sidecar
    (``<stem>.parity_sidecar.yaml``) exists next to the config, else ``actual``.
    Re-running the mapper with these args reproduces the file byte-for-byte.
    """
    feed = str((doc.get("market_data") or {}).get("feed") or "sip")
    mode = str((doc.get("simulation") or {}).get("mode") or "intended_realism")
    kind = str((doc.get("run") or {}).get("kind") or "backtest")
    purpose = "optuna" if kind == "optuna" else "backtest"
    sidecar = path.with_name(f"{path.stem}.parity_sidecar.yaml")
    feed_thresholds = "sip_tightened" if sidecar.is_file() else "actual"
    return {"feed": feed, "mode": mode, "purpose": purpose,
            "feed_thresholds": feed_thresholds}


def regenerate_generated_configs(
    configs_dir: str | Path,
    *,
    contract: dict[str, Any] | None = None,
) -> list[dict[str, Any]]:
    """Regenerate every GENERATED-marked config under ``configs_dir`` in place.

    The set of configs to regenerate is sourced from the files themselves (the
    :data:`GENERATED_MARKER` header), not a hardcoded list — so this stays
    correct as the generated-config matrix grows, and never rewrites a hand-tuned
    operator overlay (which carries no marker). Each config's own content yields
    its generation args (see :func:`recover_generation_args`), so re-running the
    mapper against the current frozen contract keeps the config in parity with it.

    Idempotent: a config already in sync with ``contract`` is rewritten
    byte-identically (``changed == False``). Returns one record per discovered
    config: ``{"path", "feed", "mode", "purpose", "feed_thresholds", "changed"}``.
    """
    ...
    for path in sorted(configs_dir.glob("*.yml")):
        try:
        ...
        if marker not in before:
            continue
        try:
            doc = yaml.safe_load(before.decode("utf-8")) or {}
        except (UnicodeDecodeError, yaml.YAMLError):
            continue
        gen = recover_generation_args(path, doc)
        import_actual_config(out_path=path, contract=contract, **gen)
        after = path.read_bytes()
        records.append({"path": str(path), **gen, "changed": after != before})
    return records
