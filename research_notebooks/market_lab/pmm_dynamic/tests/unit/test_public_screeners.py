...
def test_nonkyc_universe_normalization_from_public_payloads():
    ...
    cfg.quote_asset = "USDT"  # Explicitly test single-quote filtering
    market_list = [
        {
            "symbol": "XMR/USDT",
            "isActive": True,
...
def test_nonkyc_fetch_trades_real_api_format():
    """Verify _fetch_trades handles the actual NonKYC /market/trades response.

    Live API probe (2026-03-23) confirmed the response is a flat list with fields:
    id, price, type, baseVolume, quoteVolume, timestamp
    """
    from pmm_lab.screener.nonkyc_public import NonKYCPublicScreener, default_nonkyc_config

    # Exact shape returned by the live API
    mock_trades = [
        {
            "id": "69c1e5dedd3df620e33b76f2",
            "price": "70539.92",
            "type": "sell",
            "baseVolume": "0.026398",
            "quoteVolume": "1862.1128081600",
            "timestamp": 1774314974079,
        },
        {
            "id": "69c1e5e0dd3df620e33b7873",
            "price": "2139.14",
            "type": "buy",
            "baseVolume": "0.40646",
            "quoteVolume": "869.4748444000",
            "timestamp": 1774314976139,
        },
    ]

    cfg = default_nonkyc_config()
    screener = NonKYCPublicScreener(cfg)
    screener.client.get = lambda path, **kwargs: mock_trades

    df = screener._fetch_trades("BTC/USDT")

    assert len(df) == 2, f"Expected 2 trades, got {len(df)}"
    assert df.iloc[0]["quantity"] == 0.026398, "Should read 'baseVolume' as quantity"
    assert df.iloc[0]["price"] == 70539.92
    assert df.iloc[0]["side"] == "sell", "Should read 'type' as side"
    assert df.iloc[1]["side"] == "buy"
    assert df.iloc[0]["quote_quantity"] == 1862.11280816


def test_nonkyc_fetch_trades_empty_list():
    """Empty trade list should return empty DataFrame."""
    from pmm_lab.screener.nonkyc_public import NonKYCPublicScreener, default_nonkyc_config

    cfg = default_nonkyc_config()
    screener = NonKYCPublicScreener(cfg)
    screener.client.get = lambda path, **kwargs: []

    df = screener._fetch_trades("BTC/USDT")
    assert len(df) == 0


def test_nonkyc_fetch_trades_none_response():
    """None response should return empty DataFrame."""
    from pmm_lab.screener.nonkyc_public import NonKYCPublicScreener, default_nonkyc_config

    cfg = default_nonkyc_config()
    screener = NonKYCPublicScreener(cfg)
    screener.client.get = lambda path, **kwargs: None
