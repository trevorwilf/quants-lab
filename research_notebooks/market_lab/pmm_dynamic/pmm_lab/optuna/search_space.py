...
def suggest_params(
    ...
    fixed_quote : float, optional
        Fixed total_amount_quote. If None, optimizes it (v1 backward compat).
    ...
    # Spread ladders — log-uniform for multiplicative params
    buy_spread_base = trial.suggest_float("buy_spread_base", 0.2, 6.0, log=True)
    buy_spread_ratio = trial.suggest_float("buy_spread_ratio", 1.2, 3.0, log=True)
    sell_spread_base = trial.suggest_float("sell_spread_base", 0.2, 6.0, log=True)
    sell_spread_ratio = trial.suggest_float("sell_spread_ratio", 1.2, 3.0, log=True)

    # Amount allocation
    buy_side_weight = trial.suggest_float("buy_side_weight", 0.2, 0.8)
    amount_skew = trial.suggest_float("amount_skew", 1.0, 4.0)

    # Capital — fixed or optimized (backward compat)
    ...
    take_profit = trial.suggest_float("take_profit", 0.005, 0.15, log=True)
    ...
    trailing_stop_delta = trial.suggest_float("trailing_stop_delta", 0.001, 0.05, log=True)
