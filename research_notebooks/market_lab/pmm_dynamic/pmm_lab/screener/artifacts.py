...
def build_screening_report(run: ScreeningRun) -> str:
    ...
    lines.append(f"- Selection mode: {run.selection_mode}")
    lines.append("")
    ...
    if run.selected is None or run.selected.empty:
        ...
        cols = [c for c in [
            "trading_pair", "screen_score", "quote_volume_24h", "spread_bps",
            "top_of_book_quote", "sym_depth_quote_10bps", "sym_depth_quote_50bps",
            "sym_depth_quote_1xspread", "natr_bps_mean",
        ] if c in run.selected.columns]
...
def export_screening_artifacts(
    ...
    metadata["selection_mode"] = run.selection_mode
