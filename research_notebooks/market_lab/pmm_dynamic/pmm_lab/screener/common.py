...
class ScreenerConfig:
    ...
    min_depth_50bps_quote: float = 0.0
    min_depth_1xspread_quote: float = 0.0
    ...
    selection_mode: str = "strict"  # "strict" or "fallback_ranked"
    fallback_if_empty: bool = False  # if True, fall back to ranked when strict yields 0
    ...
    def matches_quote_asset(self, asset: str) -> bool:
        """Check whether a quote asset is accepted by this config.

        Supports:
          - Single asset:  quote_asset="USDT"       -> only USDT
          - Wildcard:      quote_asset="*"           -> all assets
          - Multi-asset:   quote_asset="USDT,XMR,BTC" -> listed assets
        """
        qa = self.quote_asset.strip()
        if qa == "*":
            return True
        accepted = {a.strip().upper() for a in qa.split(",") if a.strip()}
        return asset.strip().upper() in accepted
...
class PublicExchangeScreener:
    ...
    def screen_from_universe(self, universe: pd.DataFrame) -> ScreeningRun:
        ...
        elif self.config.fallback_if_empty and not final.empty:
            # Fallback: no pairs passed strict gates -> export top-N by score
            selection_mode = "fallback_ranked"
            selected = final.sort_values(
                ["screen_score", "quote_volume_24h"], ascending=[False, False]
            ).head(self.config.final_top_n).copy()
        finished = now_utc_iso()
        notes = [
        ...
        if selection_mode == "fallback_ranked":
            notes.append(
                "WARNING: No pairs passed strict hard gates. Selected set is ranked by screen_score "
                "(fallback_ranked mode). Review rejection_reason column before proceeding."
            )
        return ScreeningRun(
            connector=self.config.connector,
            config=self.config,
...
def compute_orderbook_metrics(
    bids: Any,
    asks: Any,
    depth_bps: Sequence[int] = (5, 10, 25, 50, 100),
) -> dict[str, Any]:
    ...
    for label in ("1xspread", "2xspread"):
        metrics[f"bid_depth_quote_{label}"] = math.nan
        metrics[f"ask_depth_quote_{label}"] = math.nan
        metrics[f"sym_depth_quote_{label}"] = math.nan
    ...
    # Spread-relative depth: depth within 1x and 2x of the half-spread from mid
    if not math.isnan(spread_bps) and spread_bps > 0:
        for mult, label in ((1.0, "1xspread"), (2.0, "2xspread")):
            band = (spread_bps * mult) / 10_000.0
            bid_floor = mid * (1.0 - band)
            ask_ceiling = mid * (1.0 + band)
            bid_depth = sum(price * qty for price, qty in bid_levels if price >= bid_floor)
            ask_depth = sum(price * qty for price, qty in ask_levels if price <= ask_ceiling)
            metrics[f"bid_depth_quote_{label}"] = bid_depth
            metrics[f"ask_depth_quote_{label}"] = ask_depth
            metrics[f"sym_depth_quote_{label}"] = min(bid_depth, ask_depth)
    else:
        for label in ("1xspread", "2xspread"):
            metrics[f"bid_depth_quote_{label}"] = math.nan
            metrics[f"ask_depth_quote_{label}"] = math.nan
...
def select_shortlist(universe: pd.DataFrame, config: ScreenerConfig) -> pd.DataFrame:
    ...
    if "quote_asset" in df.columns and config.quote_asset.strip() != "*":
        accepted = {a.strip().upper() for a in config.quote_asset.split(",") if a.strip()}
        mask &= df["quote_asset"].astype(str).str.upper().isin(accepted)
    df = df[mask].copy()
    if df.empty:
        return df
...
def build_rejection_reasons(row: Mapping[str, Any], config: ScreenerConfig) -> list[str]:
    ...
    if not config.matches_quote_asset(str(row.get("quote_asset", ""))):
    ...
    # 10bps depth gate — skip entirely when threshold is 0 (thin-venue mode)
    if config.min_depth_10bps_quote > 0:
    ...
    # 50bps depth gate (for wider-spread venues)
    if config.min_depth_50bps_quote > 0:
        depth50 = safe_float(row.get("sym_depth_quote_50bps"))
        if math.isnan(depth50):
            reasons.append("missing_depth_50bps")
        elif depth50 < config.min_depth_50bps_quote:
            reasons.append(f"depth_50bps<{config.min_depth_50bps_quote:g}")

    # Spread-relative depth gate
    if config.min_depth_1xspread_quote > 0:
        depth_sr = safe_float(row.get("sym_depth_quote_1xspread"))
        if math.isnan(depth_sr):
            reasons.append("missing_depth_1xspread")
        elif depth_sr < config.min_depth_1xspread_quote:
            reasons.append(f"depth_1xspread<{config.min_depth_1xspread_quote:g}")
...
def apply_screening_logic(df: pd.DataFrame, config: ScreenerConfig) -> pd.DataFrame:
    ...
    # Use the best available depth metric for ranking:
    # prefer 50bps if present and non-zero, else fall back to 10bps
    depth_col_50 = pd.to_numeric(out.get("sym_depth_quote_50bps", pd.Series(dtype=float)), errors="coerce")
    depth_col_10 = pd.to_numeric(out.get("sym_depth_quote_10bps", pd.Series(dtype=float)), errors="coerce")
    depth_col_sr = pd.to_numeric(out.get("sym_depth_quote_1xspread", pd.Series(dtype=float)), errors="coerce")
    # Composite: max of available bands so thin venues still rank meaningfully
    depth_composite = pd.concat([depth_col_10.fillna(0), depth_col_50.fillna(0), depth_col_sr.fillna(0)], axis=1).max(axis=1)
    out["depth_rank"] = percentile_rank(np.log1p(depth_composite))
