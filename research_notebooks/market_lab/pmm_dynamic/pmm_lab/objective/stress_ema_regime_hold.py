"""EMA regime-hold stress testing.

Symmetric to stress_mean_reversion_bb_rsi; accepts `regime_candles` too so
the strategy's compute_signals path is valid during per-scenario runs.
"""
...
def run_ema_regime_hold_fold_local_stress(
    candles: np.ndarray,
    strategy_config,
    ...
    """Run stress scenarios for EMA regime-hold on one fold's test window."""
    ...
    # Ensure strategy_config has _regime_candles wired (replace() in case it was stripped)
    if getattr(strategy_config, "_regime_candles", None) is None:
        strategy_config = replace(strategy_config, _regime_candles=regime_candles)

    initial_equity = engine_config.total_amount_quote
    candle_slice = candles[:fold_test_end_idx]

    if precomputed_signals is None:
        strategy_for_signals = EMARegimeHoldStrategy(strategy_config)
