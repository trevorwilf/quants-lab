_Source: webscrape/nonkyc/nonkyc_api_root.html (browser-rendered)_

_Loaded: 2026-04-04T04:30:31.400335Z_

![Swagger UI](data:image/svg+xml;base64,)Explore

NonKYC REST API

```
 1.3.0
```

```
OAS3
```
---------------------------------------------

</openapi.json>

### Base API url

```
https://api.nonkyc.io/api/v2/
```

### Ready to use API libraries:

GitHub: <https://github.com/NonKYC>

### Datetime Formatting

Timestamps returned as String are in ISO8601 format in UTC. Example: "2021-12-01T00:00:00Z" or "2021-12-01T00:00:00.000Z". If a timestamp is returned as a Number, then that is a Unix timestamp in milliseconds.

### Precision Formatting

All finance data such as price, quantity, and fee should be string representation of arbitrary precision numbers to prevent floating point issues. Example: "9823.23932892". Integers should be represented in integer format and not a string.

### Authentication

There are two authentication methods available for you to choose from. Basic Auth & HmacSha256 Signed Requests. Signed requests are more secure as it prevents replay attacks and accidental duplicate calls.
to make authenticated requests you must include the apiKey (public api key), nonce(Current timestamp in unix milliseconds) and signature in your request headers (X-API-KEY, X-API-NONCE and X-API-SIGN).

The signature is created by hashing the following string using the HMAC-SHA256 algorithm. The API secret (apiSecret) is used as the key for the HMAC calculation

* For Get requests : Concatenate the API Key, Request URL and Nonce into a single string.
* For Post requests : Concatenate the API Key, Request URL,Request Body and Nonce into a single string.

a python example showcasing how to send a signed request

```
import json
import requests
from time import time
import hashlib
import hmac

apiKey = "Your Api Key"
secretKey = "Your Secret key"

url = 'https://api.nonkyc.io/api/v2/cancelorder'
nonce = str(int(time()*1e3))

req_body =  {'id' :'5845454765457',}
json_body_data = json.dumps(req_body,separators=(',',':'))

data_to_sign = f"{apiKey}{url}{json_body_data}{nonce}"
signature = hmac.new(secretKey.encode(), data_to_sign.encode(),hashlib.sha256).hexdigest()

headers =  {
"X-API-KEY": apiKey,
"X-API-NONCE": nonce,
"X-API-SIGN": signature,
"Content-Type": "application/json"
}
response = requests.post(url, data=json_body_data, headers=headers)
```

### Error codes

**Error Codes**

| Error Code | HTTP Status Code | Message | Details |
| --- | --- | --- | --- |
| 400 | 400 | Unknown error | An unknown error occurred somewhere in the system |
| 402 | 400 | Method not found | The requested API method was not found |
| 403 | 401 | Action is forbidden for Account |  |
| 429 | 429 | Too many requests | Action is being rate limited for Account (Private) |
| 500 | 500 | Internal Server Error |  |
| 503 | 503 | Service Unavailable | Try it again later |
| 504 | 504 | Gateway Timeout | Check the result of your request later |
| 1001 | 401 | Authorization required |  |
| 1002 | 401 | Authorization failed |  |
| 1003 | 403 | Action is forbidden for this API key | Check permissions for API key |
| 1004 | 401 | Unsupported authorisation method | Use Basic authentication |
| 2001 | 400 | Symbol not found |  |
| 2002 | 400 | Currency not found |  |
| 10001 | 400 | Validation error | Input not valid |
| 20001 | 400 | Insufficient funds | Insufficient funds for creating order or any Account (Private) operation |
| 20002 | 400 | Order not found | Attempt to get active order that not existing, filled, canceled or expired. |
| 20003 | 400 | Limit exceeded | Withdrawal limit exceeded |
| 20004 | 400 | Transaction not found | Requested transaction not found |
| 20005 | 400 | Payout not found |  |
| 20006 | 400 | Payout already committed |  |
| 20007 | 400 | Payout already rolled back |  |
| 20008 | 400 | Duplicate clientOrderId |  |
| 20010 | 400 | Address generation error | Unable to generate a new deposit address. Try request later. |
| 20011 | 400 | Withdrawal not found | The referenced withdrawal was not found. |
| 20012 | 400 | Withdrawals disabled | Withdrawals are disabled for this currency or system wide. Check system status page. |
| 20013 | 400 | Withdrawal amount below minimum | Minimum withdrawal amount for any currency is Withdraw Fee \* 2. |
| 20014 | 400 | Withdrawal address invalid | Ensure the address you are withdrawing to is correct. |
| 20015 | 400 | Payment ID Required | This currency requires a paymentId when making a withdrawal request. |
| 20016 | 400 | Invalid confirmation code | The provided confirmation code is incorrect. |
| 20017 | 400 | Withdraw already confirmed | The withdrawal request has already been confirmed. |

Authorize

### [Assets](#/Assets) Public Asset endpoints

GET[/asset/getlist](#/Assets/asset-getlist)

List assets

GET[/asset/info](#/Assets/asset-info)

Get an asset info

### [Markets](#/Markets) Public Market endpoints

GET[/market/getlist](#/Markets/get_market_getlist)

List markets

GET[/market/info](#/Markets/market-getbyid)

Get a market info

GET[/market/orderbook](#/Markets/get_market_orderbook)

Get a Market orderbook

GET[/market/candles](#/Markets/get_market_candles)

GET[/market/trades](#/Markets/get_market_trades)

List Market trades

### [Pools](#/Pools) Public Pool endpoints

GET[/pool/getlist](#/Pools/pool-getlist)

List pools

GET[/pool/info](#/Pools/get-pool)

Get a pool info

GET[/pool/trades](#/Pools/get_pool_trades)

List pool trades

### [Account (Private)](#/Account%20(Private)) Private exchange endpoints

GET[/balances](#/Account%20(Private)/get_balances)

Get Balance

GET[/getdepositaddress/{ticker}](#/Account%20(Private)/get_getdepositaddress__ticker_)

Get deposit address

POST[/createorder](#/Account%20(Private)/post_createorder)

Create an order

POST[/cancelorder](#/Account%20(Private)/cancel-order)

Cancel an order

POST[/cancelallorders](#/Account%20(Private)/post_cancelallorders)

Cancel all open orders

POST[/createwithdrawal](#/Account%20(Private)/create-withdrawal)

Make a withdrawal

GET[/getdeposits](#/Account%20(Private)/get-deposits)

List account deposits

GET[/getwithdrawals](#/Account%20(Private)/get-withdrawals)

List account withdrawals

GET[/getorder/{orderId}](#/Account%20(Private)/get-order)

Get order info

GET[/account/orders](#/Account%20(Private)/get-orders)

List account orders

GET[/account/trades](#/Account%20(Private)/get_account_trades)

List account trades

### [Supplementary Endpoints](#/Supplementary%20Endpoints) Public exchange information

GET[/info](#/Supplementary%20Endpoints/get-exchange-info)

Get exchange Info

GET[/time](#/Supplementary%20Endpoints/get-server-time)

Get Server Time

GET[/summary](#/Supplementary%20Endpoints/get_summary)

CMC summary information for all markets

GET[/cmcassets](#/Supplementary%20Endpoints/get_cmcassets)

GET[/cmctickers](#/Supplementary%20Endpoints/get_cmctickers)

GET[/cmcorderbook/{symbol}](#/Supplementary%20Endpoints/get_cmcorderbook__symbol_)

GET[/cmctrades/{symbol}](#/Supplementary%20Endpoints/get_cmctrades__symbol_)

GET[/markets](#/Supplementary%20Endpoints/get_markets)

GET[/trades](#/Supplementary%20Endpoints/trades)

GET[/orderbook/snapshot](#/Supplementary%20Endpoints/get_orderbook_snapshot)

Get an Orderbook Snapshot

GET[/pairs](#/Supplementary%20Endpoints/get_pairs)

GET[/ticker/{symbol}](#/Supplementary%20Endpoints/get_ticker__symbol_)

GET[/tickers](#/Supplementary%20Endpoints/get_tickers)

GET[/orderbook](#/Supplementary%20Endpoints/get_orderbook)

Get Orderbook info

### [Deprecated](#/Deprecated) Deprecated endpoints

GET[/gettradessince](#/Deprecated/get_gettradessince)

Get trades since a timestamp

GET[/getpooltradessince](#/Deprecated/get_getpooltradessince)

Get pool trades since a timestamp

#### Schemas

Asset

Error

Market

LiquidityPool

OrderBook2

OrderBookItem

Candlesticks

CandlestickItem

Info

Markets

Trades

Snapshot

Pair

Ticker

OrderBook

HistoricalTrade

Balance

DepositAddress

Order

Market3

cancelallorders

Withdrawal

Deposit

Order2

Trade2

Trade

PoolTrade

LiquidityPool3

[![Online validator badge](https://validator.swagger.io/validator?url=https%3A%2F%2Fapi.nonkyc.io%2Fopenapi.json)](https://validator.swagger.io/validator/debug?url=https%3A%2F%2Fapi.nonkyc.io%2Fopenapi.json)