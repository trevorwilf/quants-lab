...
> **v11.3 update (deployable YAMLs + live-strategy health).**
> 1. **Fully deployable controller YAMLs.** Cell 9 now writes one ready-to-run
>    `range_inventory_ladder` config per qualifying market into
>    `{run}/deploy_yamls/` (+ INDEX.md), mirroring your live configs field-for-
>    field and round-trip-validated through the engine's own YAML parser.
>    Tiers: **CONFIRMED** (all gates green), **CANDIDATE** (GATED on the WF
>    process check ONLY, with holdout ≥ `rc_yaml_min_holdout_edge`, two-sided,
>    clean blocks ≥ `rc_yaml_min_clean_epr` — paper one 15d block first), and
>    **REFRESH** (pair already live: a re-fit at today's anchor — compare rungs
>    before adopting; bump the id + fresh state file). Fresh deploys seed
>    `rc_yaml_seed_frac` (50%) of the cap in QUOTE with `claimed_base_value_
>    quote: 0` (buy-first); the header carries the evidence (holdout, edge,
>    maxdd, open gates) and the first-live-block discipline.
> 2. **Live-strategy health — "no longer profitable" is now unmissable.**
>    Cell 6 triages every live YAML on its last `rc_health_recent_blocks` (3)
>    complete 15d blocks across two independent axes (absolute money, edge vs
>    hold): HEALTHY / DEFENSIVE (beating hold, losing money) / BLEEDING VS
>    HOLD (profit below doing nothing) / **NOT PROFITABLE** (losing on both —
>    `[!!] STOP` banner) / DORMANT. Saved as `{prefix}_live_health.csv`; Cell
>    11 re-runs it with days-since-last-live-fill once diagnostics are loaded.
> Test suite: 120 offline checks (incl. YAML round-trip through the real
> parser and all five health classifications).

> **v11.2 update (order-book truth, from a live API pull).** A live NonKYC
> pull settled the "DASH/USDT has $2 of depth" anomaly: the parser was right
> and the METRIC was wrong. DASH quotes dust at the touch (0.0099 DASH ~ $0.34)
> and parks its real size ~2.6% out — so +-2% of mid = **$2.40**, +-5% =
> **$29,820**, whole book = **$33,509**. The old thin-book gate was measuring
> the hole between the quotes and calling a $30k market untradeable — on
> exactly the market shape a ladder exists to trade.
> Fixes: the engine now keeps the raw BOOK (one pull, median of
> `rc_book_samples`), and depth is measured **inside the deployed ladder's own
> span** (`rc_depth_metric='ladder'`, floored at `rc_depth_min_band=5%` so a
> hair-thin ladder can't claim a hair-thin book). Gating and sizing use
> `depth_used`; new columns `depth_used`, `depth_basis`, `depth_ladder_band`,
> `book_total_usd` sit alongside the legacy `depth_2pct`. Cell 10 now prints
> the whole profile (+-2/5/10/25% + book total + your ladder's band) and takes
> the deployed ladders. Genuinely thin books still gate (regression-tested with
> the real DASH book as a fixture). Cell 6 can now label YAMLs LIVE vs DRAFT
> (`DEPLOYED_CONTROLLERS`) — an untraded config is a hypothesis, not ground
> truth. Test suite: 95 offline checks.

> **v11.1.3 update (full-system review fixes).**
> 1. **CRITICAL: daily-fallback markets were un-tradeable.** `regularize_bars6`
>    coerced *unknown* (NaN) volume to 0.0, so every Nx5-sourced bar got a zero
>    taker budget and the volume cap silently blocked ALL fills. Every
>    `granularity=1d` market (19 of 43 on the last Kraken run, incl. DASH/EUR
>    and your DASH/EUR YAML) reported 0 trades / edge 0 / SUSPECT. Fixed —
>    NaN volume now survives the grid snap (uncapped, as designed); regression
>    tests cover the sim, the report path, and NaN+known collisions.
>    **Re-run the Kraken notebook: those 19 markets were never actually evaluated.**
> 2. **Dust filter no longer starves your own markets.** The universe is built
>    without the volume floor; the floor applies to SCREENED candidates in
>    Cell 4 only. Live-YAML / always-review / focus markets are always
>    evaluated (a live ZANO/USDT ladder had been silently dropped when its
>    24h volume dipped under $10k).
> 3. **Book snapshots are now median-of-3** (`rc_book_samples`, ~1s apart) —
>    single thin-book snapshots fed the search slip, spread gate, thin-book
>    gate and sizing, and were the main source of verdict flips between
>    back-to-back runs.
> 4. Degenerate calibration windows guarded: a single live fill now yields
>    "keep logging" instead of a nonsense 10^9 fills/day rate.
> Test suite: 94 offline checks.
