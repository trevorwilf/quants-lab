...
def default_nonkyc_config() -> ScreenerConfig:
    """Conservative default gates for public NonKYC screening."""
...
class NonKYCPublicScreener(PublicExchangeScreener):
    ...
    def _build_universe_frame(
        ...
        for raw in market_list or []:
            ...
            if quote_asset.upper() != cfg.quote_asset.upper():
                continue
    ...
    def _fetch_orderbook(self, exchange_symbol: str) -> dict[str, Any]:
        ...
        try:
            return self.client.get("/market/orderbook", params=params)
        except Exception:
            return self.client.get("/orderbook", params={"ticker_id": exchange_symbol, "depth": self.config.depth_limit})
    ...
    def _fetch_trades(self, exchange_symbol: str) -> pd.DataFrame:
        ...
        for item in payload or []:
            ...
            qty = safe_float(item.get("base_volume"))
            quote_qty = safe_float(item.get("target_volume"))
            if math.isnan(price) or math.isnan(qty):
                continue
            rows.append(
                {
                    "timestamp": item.get("trade_timestamp"),
                    "price": price,
                    "quantity": qty,
                    "quote_quantity": quote_qty if not math.isnan(quote_qty) else price * qty,
